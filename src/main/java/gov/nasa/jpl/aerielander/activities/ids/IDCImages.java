...
import static gov.nasa.jpl.aerielander.models.data.DataConfig.APID.APID_IDC_2;
import static gov.nasa.jpl.aerielander.models.data.DataConfig.APID.APID_IDC_3;
import static gov.nasa.jpl.aerielander.models.data.DataConfig.APID.APID_IDC_4;
import static gov.nasa.jpl.aerielander.models.data.DataConfig.APID.APID_IDC_5;
import static gov.nasa.jpl.aerielander.models.data.DataConfig.APID.APID_IDC_6;
...
import static gov.nasa.jpl.aerielander.models.power.PowerModel.PelItem.IDC_IMAGE_EXT;
import static gov.nasa.jpl.aerielander.models.power.PowerModel.PelItem.IDC_IMAGE_PEB;

@ActivityType("IDCImages")
public final class IDCImages {
  private static final List<APID> validAPIDs = List.of(APID_IDC_1,
                                                       APID_IDC_2,
                                                       APID_IDC_3,
                                                       APID_IDC_4,
                                                       APID_IDC_5,
  ...
  @Parameter
  public APID apid = APID_IDC_6;

  @Parameter
  public int compQuality = 97;
  ...
  public IDCImages(final Duration duration, final int nFrames, final APID apid, final int compQuality) {
    this.duration = duration;
    this.nFrames = nFrames;
    this.apid = apid;
    this.compQuality = compQuality;
  }
  ...
  public void run(final Mission mission) {
    ...
    //set CLEANUP_needed["IDC"](true) immediately;
    ...
    //track_image("IDC", Name, vc, FrameSize, Nframes, CompQuality, APID);

    powerModel.setPelState(IDC_IMAGE_EXT, "on");
    powerModel.setPelState(IDC_IMAGE_PEB, "on");
    powerModel.setPelState(IDC_IDLE_EXT, "on");
    powerModel.setPelState(IDC_IDLE_PEB, "on");
    dataModel.increaseDataRate(apid, dataRate);

    delay(duration);

    dataModel.increaseDataRate(apid, -dataRate);
    powerModel.setPelState(IDC_IMAGE_EXT, "off");
    powerModel.setPelState(IDC_IMAGE_PEB, "off");
